...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _get_data(self, flag):
        data_set, data_loader = data_provider(self.args, flag)   # [Dataset: UK-Dale]
        return data_set, data_loader

    def _select_optimizer(self):
        model_optim = optim.Adam(self.model.parameters(), lr=self.args.learning_rate)   # [Optimizer: Adam]
        return model_optim
    ...
    def vali(self, vali_data, vali_loader, criterion):     # [Validation]
        total_loss = []
        self.model.eval()
        with torch.no_grad():
    ...
    def train(self, setting):              # [Training]
        ...
        early_stopping = EarlyStopping(patience=self.args.patience, verbose=True)      # [Early Stop: avoid overfitting]
    ...
    '''
    自动混合精度（Automatic Mixed Precision，AMP）是一种在深度学习训练中用于优化计算性能和内存使用的技术。
    背景和原理
    在深度学习计算中，数据类型（如float32、float16）会影响计算的精度和速度。传统上，很多模型训练使用float32（单精度浮点数）来存储模型参数和计算梯度，因为它提供了较高的精度，能够保证模型在训练过程中的稳定性。然而，float32计算相对较慢且占用更多内存。
    float16（半精度浮点数）计算速度更快，并且占用内存更少。但是它的精度较低，在某些情况下可能导致梯度计算不准确、模型收敛困难等问题。自动混合精度训练的核心思想是在模型训练过程中，自动地在float32和float16之间进行切换，以充分利用两者的优势。
    在训练过程中的具体操作（以 PyTorch 为例）
    前向传播：在模型的前向传播过程中，尽可能使用float16类型进行计算，以加快计算速度。例如，在你提供的代码中，if self.args.use_amp:条件下的with torch.cuda.amp.autocast():语句块内进行的模型预测操作，就是在自动混合精度模式下进行的。在这个块内，PyTorch 会自动将模型的输入和中间计算转换为float16类型（如果支持）进行计算。
    梯度计算和反向传播：
    由于float16在梯度计算时可能出现精度问题，在反向传播之前，需要对损失进行缩放（scale）。在代码中，scaler.scale(loss).backward()这一步就是对损失进行缩放后再进行反向传播。缩放的目的是为了避免梯度下溢（gradient underflow），因为float16的表示范围相对较小。
    经过缩放后的梯度能够在float16下相对准确地表示，并且在反向传播后，scaler.step(model_optim)用于更新模型参数。这个步骤会根据缩放后的梯度和优化器（如Adam等）来更新模型参数。
    最后，scaler.update()用于更新缩放因子（scale factor），以适应下一次迭代中的梯度缩放。
    优点
    加速训练：通过使用float16进行部分计算，可以充分利用 GPU 的张量核心（Tensor Cores）等硬件加速功能，显著提高训练速度。例如，在一些大规模的深度学习模型训练中，使用自动混合精度可以将训练时间缩短近一倍。
    节省内存：float16占用的内存只有float32的一半，这对于训练大型模型或者处理大规模数据集时非常有用。它可以使模型能够在内存有限的设备上进行训练，或者允许训练更大的批次大小（batch size），从而有可能进一步提高模型性能。
    '''
